...
<div class="collapse navbar-collapse">
	<ul class="navbar-nav mr-auto">
		...
		<a class="nav-link dropdown-toggle"  id="languages" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><fmt:message key="language"></fmt:message></a>
			...
			<a class="dropdown-item" href="?sessionLocale=en&action=userListShips"><fmt:message key="en" /></a>		
			<a class="dropdown-item" href="?sessionLocale=ua&action=userListShips"><fmt:message key="ua" /></a>
			</div>
		</li>		
...
<table class="table table-condensed table-responsive col-sm-10">
	<thead>
		<tr>
			...
			<form method="get" action="<c:url value='/customUser'/>" >
        				<input type="text" hidden name="action" value="listUserBy"/>
	...
	<tbody>
		<c:forEach var="ship" items="${userShips}"> 					
   			<tr>
    				<th><c:out value="${ship.shipID}"/></th>   
    				<th><c:out value="${ship.count}"/></th>   
       				...
       				<th><c:out value="${ship.countPort}"/></th>          
        				...
        				<input type="text" hidden name="action" value="revokeShip">
        				<input type="text" hidden name="shipID" value="${ship.shipID}">
        				<input type="text" hidden name="sessionLocale" value="${sessionScope.sessionLocale}">        				
        				<button type="submit" class="btn btn-default"><fmt:message key="revoke" /></button>  
        				</form>
        				</th>   
        				<th>        				
        				</th>         				  				     
